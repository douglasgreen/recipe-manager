...
function redirectWith(array $overrides = []): void {
    ...
    try {
        switch ($action) {
            ...
            case 'update_category':
                ...
                $name       = trim($_POST['category_name'] ?? '');
                if (!$categoryId || $name === '') {
                    flash('danger', 'Valid category and name are required.');
                } else {
                    $stmt = $pdo->prepare('UPDATE categories SET name = :name WHERE id = :id');
                    $stmt->execute([':name' => $name, ':id' => $categoryId]);
                    flash('success', 'Category renamed.');
                }
                redirectWith(['category_id' => $categoryId]);
                break;
            ...
            case 'delete_category':
                ...
                $countStmt = $pdo->prepare('SELECT COUNT(*) FROM recipes WHERE category_id = :id');
                $countStmt->execute([':id' => $categoryId]);
                if ((int)$countStmt->fetchColumn() > 0) {
                    flash('danger', 'Cannot delete: This category has recipes.');
                } else {
                    $stmt = $pdo->prepare('DELETE FROM categories WHERE id = :id');
                    $stmt->execute([':id' => $categoryId]);
                    flash('success', 'Category deleted.');
                    redirectWith(['category_id' => null]); // Reset selection
                    return;
                }
                redirectWith();
                break;
...
// Default to first category if none selected, unless list is empty
...
$recipeStmt = $pdo->prepare(implode(' ', $recipeSql));
